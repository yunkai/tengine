...
typedef struct {
    ...
    ngx_lfstack_t   idle_list[0] __attribute__((aligned(NGX_ALIGNMENT)));
    ...
    ngx_http_upstream_srv_conf_t *us)
{
    ...
    if (kcf->shared) {
        info = ngx_pcalloc(cf->pool,
                           sizeof(ngx_http_upstream_keepalive_shared_info_t));
        if (info == NULL) {
            return NGX_ERROR;
        }
        ...
        shm.size = size;
        shm.name.len = sizeof("upstream_keepalive_shm");
        shm.name.data = (u_char *) "upstream_keepalive_shm";
        shm.log = cf->log;

        /* TODO: use shm_zone to avoid memory leak when reload nginx */
        if (ngx_shm_alloc(&shm) != NGX_OK) {
            return NGX_ERROR;
        }

        ngx_memzero(shm.addr, size);
